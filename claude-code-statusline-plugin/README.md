...
**场景 1：长对话迷失**
```
❌ 问题：对话进行到第 50 轮，突然忘了"我们刚才在做什么？"
✅ 解决：状态栏实时显示 [最新指令:优化 Django 查询性能...]
```

**场景 2：多任务切换混乱**
```
❌ 问题：同时处理 3 个项目，切换终端后懵圈："这是哪个项目的任务？"
✅ 解决：一眼看到状态栏，立即想起当前任务上下文
```

**场景 3：频繁翻历史记录**
```
❌ 问题：每次想知道当前任务，都要往上滚动翻半天聊天记录
✅ 解决：关键信息永远显示在状态栏，无需翻查
```
...
> 💡 **重要说明**：此插件会安装到**全局配置目录**（`C:\Users\你的用户名\.claude\`），安装后**所有项目**都会启用状态栏。你可以在**任意目录**执行安装脚本。
